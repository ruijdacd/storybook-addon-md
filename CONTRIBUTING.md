...
End-to-end setup builds the addon once for all four Storybooks. Read-only rendering checks use two workers, then file-watching checks run sequentially to avoid changing shared fixtures during rendering tests. Screenshots are captured only on failure. Example-only button and toggle color checks are omitted; addon theme coverage remains in the browser and end-to-end suites.
