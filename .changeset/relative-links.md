...
- `links.documents` (default `true` when `links` is set) turns links to other discovered documents into manager URLs for their Docs page. Clicking one navigates the Storybook manager without reloading the preview, and opening in a new tab works.
